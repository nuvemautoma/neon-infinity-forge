...
import { Shield, Zap, Link2, RefreshCw, Headphones, Check, Crown } from "lucide-react";
import { InfinityLogo } from "@/components/InfinityLogo";

export const Route = createFileRoute("/90")({
  component: LP90,
  head: () => ({
    meta: [
      { title: "Oferta Premium 90 — Infinity I.A" },
      { name: "description", content: "Oferta premium: acesso completo a contas Plus por R$90" },
    ],
  }),
});
  ...
  return (
    <div className="min-h-screen bg-background overflow-hidden">
      ...
      <section className="relative z-10 max-w-5xl mx-auto px-6 pt-20 pb-16 text-center">
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={2} className="mt-2 flex items-center justify-center gap-2">
          <Crown className="w-5 h-5 text-yellow-400" />
          <p className="text-yellow-400 text-lg font-medium tracking-widest uppercase">Plano Premium</p>
        </motion.div>
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={3} className="mt-8 glass-strong rounded-3xl p-8 max-w-md mx-auto neon-glow-strong">
          <p className="text-muted-foreground text-sm line-through">De R$297,00</p>
          <p className="text-5xl font-extrabold gradient-neon-text mt-2">R$90,00</p>
          <p className="text-muted-foreground mt-2">Plano Premium Completo + Bônus</p>
          <div className="mt-6 space-y-2 text-left">
            {["Acesso a +30 contas premium", "Contas exclusivas VIP", "Atualizações em primeira mão", "Suporte prioritário 24/7", "Acesso vitalício", "Bônus exclusivos"].map((item) => (
